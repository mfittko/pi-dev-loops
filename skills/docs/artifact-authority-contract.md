...
- Draft gate (scope, coverage, correctness checked against PR body)
- CI green requirement
- Pre-approval gate (structural quality, docs, security)
- Merge preconditions (gate evidence, unresolved threads)
- Draft-first enforcement
- Copilot review (unless internal-only by path pattern)

### When allowed

| Allowed | Not allowed |
|---|---|
| Crash fixes (broken invocation, runtime error, missing parameter) | Feature work |
| Non-contractual doc fixes (typos, clarity, README, comments) | Contract doc changes (policy, procedure, state machine, spec) |
| Trivial corrections where PR body fully describes the change | Changes where scope/AC are ambiguous without an issue |
...
### Non-goals of the bypass

- Auto-classifying changes as trivial
- Repo-level setting to disable issue-origin requirement
- CLI flag or tool parameter accessible to subagents
- Bypassing any gate other than issue-origin and refinement
