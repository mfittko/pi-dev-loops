...
- for issue targets, do not assert "no open PR" until authoritative issue↔PR linkage is resolved via the startup resolver (`dev-loops loop startup --issue <number>`) — do not run `detect-linked-issue-pr.mjs` manually
