...
12. only after GitHub-side reply/resolve work is done for the addressed threads and the refreshed thread snapshot proves `unresolvedThreadCount === 0`, decide whether another Copilot pass is desired
    ...
    - when `stopOnLowSignal` is enabled and `copilotReviewRoundCount >= lowSignalRoundThreshold` and the last Copilot round's maximum signal level is `mid` or `low` (not `high`), the state machine returns `LOW_SIGNAL_CONVERGED` (terminal) instead of `READY_TO_REREQUEST_REVIEW`, routing to `pre_approval_gate` without further re-requests
