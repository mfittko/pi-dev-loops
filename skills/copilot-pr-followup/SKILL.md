...
12. only after GitHub-side reply/resolve work is done for the addressed threads and the refreshed thread snapshot proves `unresolvedThreadCount === 0`, decide whether another Copilot pass is desired
    ...
    - **Low-signal heuristic:** resolve low-signal config from `resolveRefinementConfig(config, "stopOnLowSignal")` / `resolveRefinementConfig(config, "lowSignalRoundThreshold")` / `resolveRefinementConfig(config, "lowSignalMaxComments")` from `@pi-dev-loops/core/config`; defaults: `stopOnLowSignal: false`, `lowSignalRoundThreshold: 3`, `lowSignalMaxComments: 2`
    - the detector classifies Copilot review-thread comments by signal level using heuristic keyword matching (no API confidence data required):
      ...
      - **Mid** — meaningful improvements, design questions, refactoring suggestions → fix once; suppress re-request when round count meets threshold
      - **Low** — cosmetic nits, phrasing preferences, trivial cleanup → fix once; do NOT re-request
    - when `stopOnLowSignal` is enabled and `copilotReviewRoundCount > lowSignalRoundThreshold` and `actionableThreadCount <= lowSignalMaxComments`, the state machine returns `LOW_SIGNAL_CONVERGED` (terminal) instead of `READY_TO_REREQUEST_REVIEW`, preventing further re-requests
    - the low-signal heuristic is applied by `detect-copilot-loop-state.mjs` / `copilot-pr-handoff.mjs` through the shared `interpretLoopState` / `summarizeLoopInterpretation` contract
