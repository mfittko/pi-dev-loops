...
- **Skip rule:** before entering the draft gate, run `detect-pr-gate-coordination-state.mjs` and check `draftGateAlreadySatisfied`. If `true`, skip the draft gate entirely — the draft→ready transition was already recorded. `draft_gate` is a one-time gate; do not re-post on new heads. This skip rule applies only to the draft boundary: once the PR is already non-draft, follow `detect-pr-gate-coordination-state.mjs` output (`gateBoundary`, `allowedNextActions`, `forbiddenActions`) rather than inferring downstream illegality from `draftGateAlreadySatisfied=false`.
