...
- the async-start contract is also enforced in code: `outer-loop.mjs` fails closed unless it detects a visible Pi-managed async run id (`PI_SUBAGENT_RUN_ID`) when repo config keeps `.pi/dev-loop/settings.yaml` `workflow.asyncStartMode: required` (default). Session-only markers (`PI_SESSION_ID`, `PI_ASYNC_CONTEXT`) and detached/local background fallback are diagnostic-only and do not satisfy startup. Snapshot/test input mode (both `--copilot-input` and `--reviewer-input`) is exempt. Standalone/test repos may opt in to `workflow.asyncStartMode: allowed`, but do not use that to route around the in-session async requirement for normal GitHub-first runs.
