...
- **Pre-delegation gate:** Before any async subagent delegation, run `<resolved-skill-scripts>/loop/copilot-pr-handoff.mjs --repo <owner/name> --pr <number>`. If it returns `action: "stop"` or `watchArgs: null`, do NOT delegate — proceed inline to the next gate instead.
...
- **Handoff template:** Use `workflow-handoff-template.md` (resolved path: `<resolved-skill-scripts>/docs/workflow-handoff-template.md`) for all subagent delegation. Include deterministic routing inputs, explicit `cwd`, bounded task scope, exit conditions, output artifact paths, and intercom coordination instructions.
