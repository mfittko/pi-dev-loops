...
- **Acceptance criteria verification:** before posting the `pre_approval_gate` comment, verify every acceptance criteria checklist item in the issue linked to this PR:
  1. resolve the linked issue number: use `gh pr view <number> --repo <owner/name> --json closingIssuesReferences,body` to extract issue references; prefer the first closing reference, then the first PR-body "Closes #N" / "Fixes #N" pattern; when exactly one issue is deterministically found, use it; when multiple candidates exist or no issue can be resolved, post the gate comment with verdict `blocked` (gate cannot complete deterministically) rather than guessing
