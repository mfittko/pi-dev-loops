...
name: issue-intake
description: >-
  Internal routed strategy behind `dev-loop` for issue-first intake and
  Copilot-owned bootstrap: resolve the authoritative issue target, confirm
  readiness and scope, apply the new-idea safety layer when needed, hand work
  to Copilot when authorized, and route into PR follow-up once a linked PR is
  ready.
...
This skill is the canonical internal `issue_intake` route behind the public `dev-loop` façade.

Use it only when the public `dev-loop` router lands on an issue-first GitHub/Copilot path. Keep repository specifics grounded in the active repo's actual files, scripts, CI, and GitHub state rather than assuming a hard-coded project layout.

This route owns issue refinement, the new-idea safety layer, Copilot-first bootstrap, and the handoff into PR follow-up once a linked PR is ready.
...
- if you are working in the `pi-dev-loops` source repository, this skill file lives under `skills/issue-intake/`, so source-repo helper scripts live two levels up at `../../scripts/`, while required bundled contract docs live one level up at `../docs/`
