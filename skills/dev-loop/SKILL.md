...
**Async dispatch rule (#465, enforced):** the resolver enforces this fail-closed for GitHub-first strategies (`issue_intake`, `copilot_pr_followup`, `external_pr_followup`, `reviewer_fixer`, `wait_watch`). Inline invocation without `PI_SUBAGENT_RUN_ID` is rejected with exit code 1 (stderr JSON: `{"ok":false,"asyncStartContract":"rejected"}`). Bypass for development: `PI_ASYNC_START_BYPASS=1`. For any routed strategy where the resolver's `canonicalStateSummary.requiresAsyncDispatch` is `true`, dispatch the strategy as a single async coordinator subagent (the `dev-loop` agent) rather than executing steps inline in the parent session. The dispatched agent owns parallel review fan-out, fixer passes, gate comments, state transitions, and sub-delegation internally.
