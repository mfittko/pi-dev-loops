...
const REMOTE_READINESS_IDS = ["gh-installed", "gh-auth", "subagent-tool", "git-repo", "local-dev-loop-skill"];
...
function devLoopSkillMetadata(surface = "extension") {
...
export async function collectDevLoopChecks(runtime, { surface = runtime?.surface ?? "extension" } = {}) {
  const skillMetadata = devLoopSkillMetadata(surface);
  const [ghInstalled, ghAuthenticated, inGitRepo, subagentProbe, localSkillProbe] =
    await Promise.all([
  ...
  const localSkill = normalizeProbe(
    ...
    skillMetadata.unavailableDetail,
  );
