...
export function parseDevLoopsCommand(input, { surface = "extension" } = {}) {
  ...
  switch (action) {
    ...
    case "":
    case "help":
      return rest.length === 0 && rawScope === undefined
        ? { kind: "action", action: "help", tokens }
        : invalidCommand("`help` does not accept additional arguments.", undefined, tokens);
    case "status":
    case "doctor":
      return rest.length === 0 && rawScope === undefined
        ? { kind: "action", action, tokens }
        : invalidCommand(`\`${action}\` does not accept additional arguments.`, undefined, tokens);
    case "hide":
      if (rest.length > 0 || rawScope !== undefined) {
        return invalidCommand("`hide` does not accept additional arguments.", undefined, tokens);
      }
    ...
    default:
      return invalidCommand(`Unrecognized command: ${rawAction}.`, undefined, tokens);
  }
}
