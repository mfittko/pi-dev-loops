...
`dev-loops` is a private, source-loaded workspace for reusable Pi development loops.
...
Installing the package with `pi install git:github.com/mfittko/dev-loops` exposes:
- the `/dev-loops` extension command surface
- the `dev-loops` shell CLI
- the packaged skills from `package.json` `pi.skills`

For project-local installs, use `pi install -l git:github.com/mfittko/dev-loops`.
