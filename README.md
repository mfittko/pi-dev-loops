...
CI currently runs `npm ci`, `npm run verify`, restores a workspace-local Playwright WebKit runtime cache keyed by runner OS + resolved `@playwright/test` version, and runs the explicit Playwright viewer smoke in `.github/workflows/ci.yml`.
