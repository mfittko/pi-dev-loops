...
// ---------------------------------------------------------------------------
// evaluateRetrospectiveMergeGate — merge-gate enforcement
// ---------------------------------------------------------------------------

import { evaluateRetrospectiveMergeGate } from "../src/loop/retrospective-checkpoint.mjs";

test("evaluateRetrospectiveMergeGate: passes through when requireRetrospectiveGate is false", () => {
  const proposed = makeCopilotPrFollowupResult();
...
test("evaluateRetrospectiveMergeGate: blocks when MISSING checkpoint", () => {
  ...
  assert.equal(result.routeKind, DEV_LOOP_ROUTE_KIND.NEEDS_RECONCILE);
  assert.equal(result.selectedGate, "fail_closed_reconcile");
  assert.match(result.reason, /retrospective/i);
});

test("evaluateRetrospectiveMergeGate: blocks when mergeApproved is false", () => {
  const proposed = makeCopilotPrFollowupResult();
  const result = evaluateRetrospectiveMergeGate({
    requireRetrospectiveGate: true,
    checkpointState: RETROSPECTIVE_CHECKPOINT_STATE.COMPLETE,
  ...
  assert.equal(result.routeKind, DEV_LOOP_ROUTE_KIND.NEEDS_RECONCILE);
  assert.match(result.reason, /merge.*approved/i);
});

test("evaluateRetrospectiveMergeGate: blocks when checkpointState is NONE", () => {
  const proposed = makeCopilotPrFollowupResult();
  const result = evaluateRetrospectiveMergeGate({
    requireRetrospectiveGate: true,
    checkpointState: RETROSPECTIVE_CHECKPOINT_STATE.NONE,
...
test("evaluateRetrospectiveMergeGate: does not block stop results", () => {
  const proposed = evaluatePublicDevLoopRouting({
    intent: DEV_LOOP_PUBLIC_INTENT.CONTINUE_CURRENT,
    currentState: {
      target: { kind: DEV_LOOP_TARGET_KIND.PR, pr: 88 },
