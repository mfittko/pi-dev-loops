...
test("tracker-first MVP state graph is documented as adapter-agnostic and bounded", async () => {
  ...
  assert.match(content, /Inh(?:erit|erited) from `#21` \(not redefined here\)/i);
  assert.match(content, /If any statement in this file appears to conflict with `#21`.*`#21` is authoritative/i);
  assert.match(content, /one work-item -> one PR invariant/i);
  assert.match(content, /reverse-sync semantics and tracker-link meaning/i);
  assert.match(content, /epic\/PRD\/ADR\/RFC/i);
  assert.match(content, /narrower than.*`#19`/i);
  assert.match(content, /Ordered interpretation rules \(first match wins\)/i);
  assert.match(content, /one current state/i);
  assert.match(content, /first satisfied rule selects/i);
  assert.match(content, /reviewable_pr[\s\S]*!prReviewRequested/i);
  assert.match(content, /waiting_for_review[\s\S]*prReviewRequested/i);
  assert.match(content, /waiting_for_ci[\s\S]*ciStatus === "pending"/i);
  ...
  assert.match(content, /requiredSyncLinksPresent/i);
  assert.match(content, /required by `#21`/i);
