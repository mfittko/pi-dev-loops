...
test("public dev-loop contract keeps tracker-backed local work inside local_implementation", async () => {
  const [publicContract, devLoopSkill] = await Promise.all([
    readRepo("skills/docs/public-dev-loop-contract.md"),
    readRepo("skills/dev-loop/SKILL.md"),
  ]);
  ...
  assert.match(devLoopSkill, /Tracker-backed local implementation/i);
  assert.match(devLoopSkill, /stays inside the existing `local_implementation` path/i);
  assert.match(devLoopSkill, /do not create or read `docs\/phases\/phase-x\.md` for that same tracker-backed session/i);
  assert.match(devLoopSkill, /sync durable scope \/ acceptance \/ status changes back to the tracker issue/i);
});
