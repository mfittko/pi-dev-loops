...
test("help is the default action and malformed commands stay non-mutating", async () => {
  ...
  assert.equal(statusWithExtraArgsContext.calls.notifications.at(-1).message, "pi-dev-loops status: 5/5 checks passed");
...
test("status keeps remote readiness ready when the public dev-loop skill is installed", async () => {
...
test("install repo copies packaged skills into the repository, repo errors stay action-specific, and hide still clears the widget", async () => {
  ...
  await access(path.join(repoRoot, ".pi", "skills", "copilot-autopilot", "scripts", "loop", "detect-initial-copilot-pr-state.mjs"));
