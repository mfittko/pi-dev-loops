...
In **tracker-backed mode**, this file must NOT exist and must NOT be read. The tracker issue body is the canonical spec and the two are mutually exclusive. If a phase doc exists, fail closed (stop and reconcile). See `skills/docs/tracker-backed-local-contract.md`.
