...
| Field | Meaning |
|---|---|
| `target` | active artifact: `issue` \| `pr` \| `local_branch` \| `local_phase` |
...
6. issue target -> `issue_intake`
7. PR owned by external human -> `external_pr_followup`
8. PR owned by reviewer or next actor reviewer -> `reviewer_fixer`
9. PR owned by Copilot -> `copilot_pr_followup`
10. anything else -> fail closed to `needs_reconcile`
...
|---|---|
| start dev loop on issue `86` | synthesize issue target -> `issue_intake` -> compatibility `copilot-autopilot` |
