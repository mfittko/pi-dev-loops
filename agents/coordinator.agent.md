...
- The default pre-approval review fan-out must use the review angles resolved from config (`resolveGateAngles(config, "preApproval")`) in parallel when practical before calling work review-complete, approval-ready, merge-ready, or ready for final handoff.
...
11. For that review fan-out, resolve angles from config (`resolveGateAngles(config, "preApproval")`) and for each angle resolve its persona and prompt via `resolveReviewerRole(config, angle)`. Include the angle-specific prompt in each reviewer's briefing. Run the reviews in parallel when practical. If parallel execution is impractical (for example due to tooling or resource constraints), still run all configured angles and record that limitation explicitly in the reviewer handoff artifact or merged review verdict.
